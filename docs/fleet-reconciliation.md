...
outside `dinglebear-ai/workflows`. This library now provides 42 reusable
...
| `ci-runner-farm` | 4 | fast ops, Unraid plugin CI/release, Release Please, hosted release command | upstream-fork compatibility and self-contained `.plg` assembly contract |
...
| `incus-web` | 1 | fast Node, hosted release command, container release | Incus image format and deployment destination |
| `labby` | 7 | all fast language/ops lanes, labeler/stale/drift, MCP conformance, Android/container/platform/Incus-style release | trusted classifier, Windows/custom palette graph, upstream API semantics |
